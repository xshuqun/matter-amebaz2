...
u8 matter_wifi_trigger;
void matter_wifi_autoreconnect_hdl(rtw_security_t security_type, char *ssid, int ssid_len, char *password, int password_len, int key_id);
...
void matter_wifi_set_autoreconnect(u8 mode)
{
	...
	return rltk_wlan_set_autoreconnect(WLAN0_NAME, mode, AUTO_RECONNECT_COUNT, AUTO_RECONNECT_INTERVAL);
}
...
void matter_check_ip6_addr_isvalid()
{
